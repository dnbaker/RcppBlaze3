# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#' linear model fitting function based on RcppBlaze3
#'
#' \code{fastLmPure} provides the estimates of the linear model based on \code{RcppBlaze3}.
#'
#' \code{fastLm} estimates the linear model using the \code{solve}.
#'
#' @param X A model matrix.
#' @param y A response vector.
#' @param type A integer. 0 is QR solver, 1 is LLT solver and 2 is LDLT sovler.
#' @return A list containing coefficients, standard errors, rank of model matrix,
#'   degree of freedom of residuals, residuals, the standard deviation of random errors and
#'   fitted values.
#' @examples
#' # according to fastLm example in RcppArmadillo
#' data(trees, package="datasets")
#' flm <- fastLmPure(cbind(1, log(trees$Girth)), log(trees$Volume), 0)
#' print(flm)
#' @export
fastLmPure <- function(X, y, type) {
    .Call('RcppBlaze3_fastLmPure', PACKAGE = 'RcppBlaze3', X, y, type)
}

Blaze3_version <- function(single) {
    .Call('RcppBlaze3_Blaze3_version', PACKAGE = 'RcppBlaze3', single)
}

Blaze3_SSE <- function() {
    .Call('RcppBlaze3_Blaze3_SSE', PACKAGE = 'RcppBlaze3')
}

Blaze3_AVX <- function() {
    .Call('RcppBlaze3_Blaze3_AVX', PACKAGE = 'RcppBlaze3')
}

Blaze3_AVX2 <- function() {
    .Call('RcppBlaze3_Blaze3_AVX2', PACKAGE = 'RcppBlaze3')
}

Blaze3_MIC <- function() {
    .Call('RcppBlaze3_Blaze3_MIC', PACKAGE = 'RcppBlaze3')
}

Blaze3_FMA <- function() {
    .Call('RcppBlaze3_Blaze3_FMA', PACKAGE = 'RcppBlaze3')
}

